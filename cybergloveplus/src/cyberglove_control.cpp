...
int CyberGloveControl::init() {
  ...
  static const std::string joint_names[20] = {
      "FFJ0", "FFJ3", "FFJ4", "MFJ0", "MFJ3", "MFJ4", "RFJ0",
  ...
  for (int i = 0; i < JOINTS_SIZE; i++) {
    ...
    publisher[joint] =
        n_tilde.advertise<std_msgs::Float64>(prefix + joint_temp + suffix, 2);
  }

  robot_state_publisher = n_tilde.advertise<moveit_msgs::DisplayRobotState>(
      prefix + "robot_state", 2);
...
void CyberGloveControl::run() { CyberGloveCalibMinMax::run(); }
...
void CyberGloveControl::send_commands() {
  ...
  for (auto &p : j_commands) {
    // display_robot_state.state.joint_state.name.push_back(p.first);
    // display_robot_state.state.joint_state.position.push_back(p.second);
    if (p.first.size()) {
      if (p.first[p.first.size() - 1] == '0') {
        std::string name = p.first;
        name.resize(name.size() - 1);
        display_robot_state.state.joint_state.name.push_back("rh_" + name +
                                                             "1");
        display_robot_state.state.joint_state.position.push_back(p.second *
                                                                 0.5);
        display_robot_state.state.joint_state.name.push_back("rh_" + name +
        ...
        display_robot_state.state.joint_state.name.push_back("rh_" + p.first);
        display_robot_state.state.joint_state.position.push_back(p.second);
      }
    }
  }
