...
int CyberGloveCalibMinMax::init() {
  ...
  calib_sub = n_tilde.subscribe(
      full_topic, 1000, &CyberGloveCalibMinMax::command_received_callback,
      this);
...
void CyberGloveCalibMinMax::command_received_callback(
    const cybergloveplus::Calibration::ConstPtr &msg) {
  ...
  for (unsigned int i = 0; i < msg->name.size(); i++) {
    ...
    if (min_values.find(sensor) == min_values.end()) {
      ...
      if (msg->auto_calibration[i])
        ROS_INFO("Command for %s auto enabled min=%f max=%f", sensor.c_str(),
                 msg->min[i], msg->max[i]);
      else
        ROS_INFO("Command for %s auto disabled min=%f max=%f", sensor.c_str(),
                 msg->min[i], msg->max[i]);
...
void CyberGloveCalibMinMax::calibrate() {
  for (int i = 0; i < GLOVE_SIZE; i++) {
    ...
    if (auto_calibration[sensor]) {
      if (gl_values[sensor] < min_values[sensor])
        min_values[sensor] = gl_values[sensor];
      if (gl_values[sensor] > max_values[sensor])
        max_values[sensor] = gl_values[sensor];
    }

    gl_calibrated_values[sensor] = (gl_values[sensor] - min_values[sensor]) /
                                   (max_values[sensor] - min_values[sensor]);
    if (!std::isfinite(gl_calibrated_values[sensor])) {
      gl_calibrated_values[sensor] = 0.0;
    }
  }
}
