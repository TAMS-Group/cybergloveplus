...
int CyberGloveRaw::open_serial(std::string port) {
  ...
  serial_port_fd = open(
      port.c_str(),
      O_RDWR |
          O_NOCTTY); // will return a file descriptor based on an actual file
  ...
  //! trying the following lines to restart serial port, see read_stepping()
  //! function
...
*	b is length GLOVE_SIZE + 2 because the first two characters returned are
G and ' '
*/
...
void CyberGloveRaw::read_sensor_values() {
  while (1) {
    ...
    //  error(0,0,"res=%d, %02x %02x %02x %02x %02x %02x", res,
